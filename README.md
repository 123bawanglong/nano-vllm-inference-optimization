...
![image-20260921181349154](docs/images/image-20260921181349154.png)

再按调用次数排序：归一化 kernel 调用频繁，列为融合候选；是否属于 Q/K RMSNorm，继续结合时间线和源码确认。

![image-20260921181431507](docs/images/image-20260921181431507.png)


在Decode 时间线和源码确认：Q Norm、K Norm、Q RoPE、K RoPE 分开连续执行，是可以进行融合的重要候选

![image-20260921183540731](docs/images/image-20260921183540731.png)


|候选|判断|
|---|---|
|矩阵计算、Attention|分别占 Decode GPU kernel 累计耗时约66.40%～76.08%、13.69%～23.32%；已有成熟实现，本轮结合改动成本暂缓，不代表没有优化空间|
|Q/K Norm→RoPE|占 Decode GPU kernel 累计耗时3.12%～3.72%；每步112个节点，存在可消除的中间读写|
|Add+Norm、SiLU×up、采样|部分计算已由 torch.compile 融合；需分别检查剩余边界，不能认定整个采样链已合为一个 kernel|

## 2．NCU分析

Roofline：点远低于上界；计算强度低，还不能据此断言 DRAM 带宽打满。

![image-20260921174852958](docs/images/image-20260921174852958.png)


Speed of Light：四个节点 Compute仅0.11%～0.25%、DRAM仅1.04%～4.52%，先关注小任务的开销。

![image-20260921174903132](docs/images/image-20260921174903132.png)


Compute Workload Analysis：Q Norm 的 Executed IPC Active约0.08，没有计算管线饱和的证据。

![image-20260921174941710](docs/images/image-20260921174941710.png)


Memory Workload Analysis：结合源码，Norm中间结果每层每token逻辑上写6 KiB再读6 KiB；融合可省这段传递。

![image-20260921175005623](docs/images/image-20260921175005623.png)


Scheduler Statistics：Q Norm 的 Eligible约0.04、No Eligible约95.71%，可发射工作很少。

![image-20260921175046966](docs/images/image-20260921175046966.png)


Warp State Statistics：存在 Long Scoreboard 访存依赖等待；需结合指令和数据依赖分析，不能单凭 stall 决定融合。

![image-20260921175100817](docs/images/image-20260921175100817.png)


Source / SASS：实际指令包含SHFL.BFLY归约、MUFU.RSQ和BF16转换，融合时要保留对应数值语义。

![image-20260921175150662](docs/images/image-20260921175150662.png)
...
![image-20260921183946242](docs/images/image-20260921183946242.png)
...
![image-20260921184121401](docs/images/image-20260921184121401.png)
...
9组局部微基准数值全部对齐，Norm＋RoPE 整段延迟下降67.24%～73.08%；这是局部收益，不是整模型加速幅度。

![image-20260921184312895](docs/images/image-20260921184312895.png)


融合后Roofline仍远低于上界；我用独立计时验证收益，不用NCU的3.328 μs冷重放时间计算加速比。

![image-20260921184932098](docs/images/image-20260921184932098.png)


端到端：固定10组配对、280次请求、全部保留

![image-20260921185015602](docs/images/image-20260921185015602.png)
...
5种配置的描述性95%区间为正，两种B=1、输出256的配置区间跨零；桌面GPU未锁频，未观察到PyTorch allocated显存下降。

## 5．融合后 NCU 原图复核

Speed of Light：融合后Compute约0.26%、DRAM约3.03%，仍未接近整卡上限。

![image-20260921185251509](docs/images/image-20260921185251509.png)


Compute Workload Analysis：复核融合后的指令和管线压力，不能把四个原节点与一个融合节点的吞吐比当成端到端加速比。

![image-20260921185238948](docs/images/image-20260921185238948.png)


Memory Workload Analysis：中间全局数组被消除；逻辑字节节省不等于同量DRAM流量节省。

![image-20260921185222818](docs/images/image-20260921185222818.png)


Scheduler Statistics：融合后Eligible约0.05，仍是小任务，收益不等于GPU已经满载。

![image-20260921185208074](docs/images/image-20260921185208074.png)


Warp State Statistics：融合后仍有依赖等待；我保留这一限制，没有把所有stall下降当作成功条件。

![image-20260921185149330](docs/images/image-20260921185149330.png)


Source / SASS：源码保留BF16舍入

![image-20260921185125015](docs/images/image-20260921185125015.png)
...
[融合 kernel](src/qk_norm_rope/kernel.cu) · [模型接入](src/qk_norm_rope/adapter.py)
