...
def main(out):
    ...
    lines+=['','## 3. 再看调用次数与可消除的开销','',
        ...
        '', '## 5. 本轮决策及验收边界','',
        '综合本项目由用户亲自写核心kernel、需要接入真实推理且能解释数值的目标，选择 **Q/K RMSNorm→RoPE** 作为第一版原型是合理的：开销在完整执行中可测且重复出现，中间消费者唯一，能明确陈述要减少什么，实现范围小于替换线性层/attention。采样是值得保留的竞争方向，其正确性契约更复杂。',
        '', '该选择不代表全局最优，也没有证明它能提升端到端性能。矩阵产品和attention占据主要耗时；最终Norm选行则是额外发现的低成本小预算机会。若将目标改成最大化模型吞吐而不约束开发成本，应优先深入矩阵/attention效率调查。',
        '', '原型必须以实际compiled baseline为参考：Python中写了cast，不代表编译生成代码保留每一次舍入。核心CUDA仍由用户实现；本轮没有自写kernel，也未改变数学计算或采样方法。正确性、微基准和端到端A/B属于实现后的验收。',
