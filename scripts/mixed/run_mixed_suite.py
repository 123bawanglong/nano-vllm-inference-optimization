...
def run(backend, budget, graph, action, label=''):
    ...
    cmd = [sys.executable, str(REPO/'scripts/mixed/bench_mixed.py'), '--backend', backend,
...
def main():
    ...
    parser.add_argument('--action', choices=['correctness', 'benchmark'], required=True)
    args = parser.parse_args()
    DEST.mkdir(parents=True, exist_ok=True)
    if args.action == 'correctness':
        for budget, graph in ((128, 1), (512, 1), (1024, 1), (512, 0)):
            for backend in ('fusion', 'mixed'):
                run(backend, budget, graph, args.action)
        ...
        for budget in (128, 512, 1024):
            ...
            for label, backends in (('ab', ('fusion', 'mixed')), ('ba', ('mixed', 'fusion'))):
                for backend in backends:
                    run(backend, budget, 1, args.action, label)
