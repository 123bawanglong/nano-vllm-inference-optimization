...
ROOT = REPO/'src/nano-vllm-cuda-mixed'
...
def main():
    ...
    parser.add_argument('--backend', choices=['fusion', 'mixed'], required=True)
    ...
    os.environ['NANOVLLM_FUSED_OPS'] = 'all'
    # Separate build caches avoid rebuilding identical sources when copies alternate.
    os.environ.setdefault('TORCH_EXTENSIONS_DIR', str(Path.home()/'.cache/nanovllm-scheduler-eval'/args.backend))
    backend_root = ROOT if args.backend == 'mixed' else ROOT.parent/'nano-vllm-cuda'
    ...
    probe = Probe(engine)
    dest = Path(os.environ.get('NANOVLLM_RESULTS_DIR', str(REPO/'results/mixed')))
